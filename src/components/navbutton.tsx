'use client'
import React from 'react';
import Image from 'next/image';
import Link from 'next/link';
const NavButton = () => {
    return (
        <div className='flex mt-10 'style={{ marginLeft: "550px" }}>
<<<<<<< HEAD:src/components/admin_login/navbutton.tsx
        <Link href="/login">
            <button className='flex justify-center items-center w-36 h-10 border-2 border-black rounded-md hover:bg-gray-500' >
            <Image className='pl-2 pt-2'
=======
            <button className='flex justify-center items-center w-36 h-12 border-2 border-black rounded-md hover:bg-gray-500' >
            <Image className=' pt-1 w-8'
>>>>>>> upstream:src/components/navbutton.tsx
        src="/Arrow 5.svg"
        alt="Next.js logo"
        width={40}
        height={28}/>
<<<<<<< HEAD:src/components/admin_login/navbutton.tsx
    <h1 className='ml-4 text-2xl font-semibold'> Back</h1> </button>
        </Link>
        
    <button className='flex  w-44 h-10 border-2 border-black rounded-md hover:bg-gray-500 bg-black text-white ml-12' >
              <h1 className='ml-4 text-2xl font-semibold'>Login </h1>
              <Image className='pl-2 pt-1 ml-6 mb-4'
=======
    <h1 className='ml-2 text-2xl font-semibold'> Back</h1> </button>
    <button className='flex justify-center items-center w-64 h-12 border-2 border-black rounded-md hover:bg-gray-500 bg-black text-white ml-12' >
              <h1 className='ml-4 text-2xl font-semibold '>Login </h1>
              <Image className='pl-2 pt-1 ml-2 '
>>>>>>> upstream:src/components/navbutton.tsx
        src="/Arrow 6.svg"
        alt="Next.js logo"
        width={40}
        height={20}
        
      /> </button>
        
              
        </div>
    );
}
export default NavButton;